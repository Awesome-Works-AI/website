...
- Social profiles (`github.com/Raff-dev`, `x.com/awesomeworksai`, `linkedin.com/in/rlazicki`, `instagram.com/makeitagentic`) are visible on-page and canonically defined in `src/data/personal-home.ts` (top of file). These are off-site corroboration surfaces already in existence.
...
- Four social profile URLs are already defined in `src/data/personal-home.ts` and are visible on-page: GitHub, X (`x.com/awesomeworksai`), LinkedIn (`linkedin.com/in/rlazicki`), Instagram (`instagram.com/makeitagentic`). These could be passed into the Organization `sameAs` immediately without inventing new claims.
...
| Instagram `instagram.com/makeitagentic` | Exists, linked on-page | Weak citation surface; appropriate for social discovery only |
