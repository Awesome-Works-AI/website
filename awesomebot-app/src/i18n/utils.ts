...
/**
 * Astro's configured base path (e.g. `/awesomebot/` when this app is mounted
 * under awesomeworks.ai/awesomebot). `import.meta.env.BASE_URL` always ends
 * with `/` per Astro spec.
 */
...
function stripBase(pathname: string): string {
  ...
  const trimmed = BASE.replace(/\/$/, ''); // "/awesomebot"
...
function withBase(path: string): string {
  ...
  const trimmedBase = BASE.replace(/\/$/, ''); // "/awesomebot"
